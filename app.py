...
def submit():
    try:
        ...
        # --- Step 1: Translate the user input ---
        translated_input = translate_text(user_input, source="auto", target=lang)

        # --- Step 2: Chatbot response using translated input ---
        ...
        # --- Step 3: TTS output ---
        tts_filename = ""
        print("🧠 Preparing to synthesize speech for:", chatbot_reply)
        ...
        if lang in LANG_VOICE_MAP:
            ...
            print("🗣️ Using voice:", LANG_VOICE_MAP[lang])
            asyncio.run(generate_tts_edge(chatbot_reply, LANG_VOICE_MAP[lang], output_path))
        else:
            print("⚠️ Language not found in LANG_VOICE_MAP")
...
def news():
    try:
        ...
        news_response = requests.get(
            ...
            params={"country": "us", "apiKey": os.getenv("NEWSAPI_API_KEY")}
